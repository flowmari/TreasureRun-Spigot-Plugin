...
if not LANG_JSON_DIR.exists():
    print(f"ERROR: Run from TreasureRun-main/"); sys.exit(1)
YAML_TO_JSON = {
...
TARGET_CATS = {
    "painting","container","structure_block","trim_pattern","trim_material",
    "instrument","dataPack","jigsaw_block","gameMode","pack","datapackFailure",
    "event","inventory","mirror","clear","difficulty","editGamerule","upgrade",
    "hanging_sign","record","sign",
}
def load_json(lang):
...
def process(path, lang):
    ...
    new_kv = {k: str(v) for k,v in jd.items() if k.split(".")[0] in TARGET_CATS and k.split(".")[0] not in ec}
...
print(f"\nStep 3 complete. Total keys added: {total}")
