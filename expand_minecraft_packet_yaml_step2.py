...
def process(path, lang):
    ...
    target = {k: str(v) for k,v in jd.items()
              if k.startswith("item.minecraft.") or k.startswith("block.minecraft.")}
    content = path.read_text(encoding="utf-8")
    ec = existing_cats(content)
    new_kv = {k:v for k,v in target.items() if k.split(".")[0] not in ec}
    if not new_kv: print(f"  [{lang}] item/block already present"); return 0
    ...
    print(f"  [{lang}] +{len(new_kv)} item/block keys")
    return len(new_kv)
total = 0
for p in sorted(YAML_DIR.glob("*.yml")):
    total += process(p, p.stem)
print(f"\nStep 2 complete. Total item/block keys added: {total}")
